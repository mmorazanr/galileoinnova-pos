"""
Importa el SalesDayNumber=6349 como 2026-02-24 desde Marzo2026/CBOData_s.mdb
(turno nocturno que abrio el 23/02 pero opera como dia de negocio 24/02)
"""
import json, pyodbc, mysql.connector, datetime
...
remote_conn = mysql.connector.connect(
    ...
    password=cfg['remote_db_pass'], database=cfg['remote_db_name'], use_pure=True
)
local_conn = pyodbc.connect(
    "Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=C:\\ICOM\\Database\\Marzo6026\\CBOData_s.mdb;PWD=C0mtrex;"
)
cur = local_conn.cursor()
push = remote_conn.cursor()

s_day = 6349
fecha_str = '2026-02-24'

print(f"Importando SalesDayNumber={s_day} como {fecha_str}...")

cashier_map = {}
cur.execute("SELECT DISTINCT CashierNumber, CashierName FROM DailyCashierSales")
for row in cur.fetchall():
    cashier_map[str(row[0])] = row[1]

category_map = {}
try:
    cur.execute("SELECT DISTINCT CategoryNumber, CategoryName FROM K_CategoryDefinitions")
    for row in cur.fetchall():
        category_map[str(row[0])] = row[1]
except Exception:
    try:
        cur.execute("SELECT DISTINCT CategoryNumber, CategoryName FROM DailyCashierSales")
        for row in cur.fetchall():
...
for row in rows:
    ...
    batch.append((cfg['restaurant_name'], trans_id, fecha_str, '12:00:00',
                  cashier_map.get(cid, f'Cajero {cid}'),
                  category_map.get(catid, f'Cat {catid}'),
                  row[4], float(row[5] or 0), float(row[6] or 0)))
if batch:
    push.executemany("""INSERT INTO restaurantes_ventas
...
cur.execute(f"SELECT MediaName, SUM(POSAmount) FROM DailyCashierMedia WHERE SalesDayNumber={s_day} GROUP BY MediaName")
for r in cur.fetchall():
    m = (r[0] or '').strip()
    if m in mapping: metrics[mapping[m]] = float(r[1] or 0)
cur.execute(f"SELECT MiscDescription, SUM(MiscAmount) FROM DailyCashierMisc WHERE SalesDayNumber={s_day} GROUP BY MiscDescription")
for r in cur.fetchall():
    d = (r[0] or '').strip()
...
print(f"  restaurantes_diario_media: net_sales={metrics['net_sales']}, cash={metrics['cash']}")

remote_conn.commit()
local_conn.close()
remote_conn.close()
print(f"\nDia {fecha_str} importado correctamente.")
